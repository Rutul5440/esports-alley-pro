  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        ...
        <p className="text-muted-foreground mt-1">Manage your profile, game preferences, and account.</p>
        ...
        <section className="mt-8 rounded-xl border border-border bg-card p-6 space-y-5">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <img
                src={avatarFile ? URL.createObjectURL(avatarFile) : playerProfile?.user?.avatar || `https://api.dicebear.com/9.x/initials/svg?seed=${user?.username}`}
                alt=""
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{isOrg ? "Organization" : "Player"} account</p>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Country / Region</span>
          ...
          <label className="flex items-center justify-between rounded-md border border-border p-3 text-sm">
            ...
            <input type="checkbox" checked={isOpenToTeam} onChange={(e) => setIsOpenToTeam(e.target.checked)} className="h-4 w-4 accent-primary" />
          </label>
        </section>
        ...
        <section className="mt-6 rounded-xl border border-border bg-card p-6 space-y-5">
          ...
          <p className="text-sm text-muted-foreground">Select games you actively play. This affects scouting visibility and feed filtering.</p>
        ...
        <section className="mt-6 rounded-xl border border-border bg-card p-6 space-y-5">
          ...
          <div className="grid gap-3 text-sm">
            ...
            <div className="flex items-center justify-between rounded-md border border-border p-3">
              <span>Role</span>
              <span className="rounded-md bg-primary/10 px-2 py-0.5 text-xs text-primary">{user?.role}</span>
            </div>
          </div>
        </section>
