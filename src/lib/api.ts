...
export const authApi = {
  login: (email: string, password: string) =>
    unwrap(api.post<ApiResponse<AuthPayload>>("/auth/login", { email, password })),
  register: (payload: {
    username: string;
    email: string;
    password: string;
  ...
  checkUsername: (username: string) =>
    unwrap(
      api.post<ApiResponse<{ username: string; available: boolean }>>("/auth/check-username", {
        username,
      }),
    ),
  completeProfile: (payload: Record<string, unknown>) =>
    unwrap(api.put<ApiResponse<AuthPayload>>("/auth/complete-profile", payload)),
  follow: (userId: string) => unwrap(api.post<ApiResponse<null>>(`/auth/follow/${userId}`)),
  uploadAvatar: (file: File) => {
    const fd = new FormData();
    fd.append("avatar", file);
    return unwrap(
      api.post<ApiResponse<{ url: string }>>("/auth/avatar", fd, {
  ...
  uploadBanner: (file: File) => {
    ...
    return unwrap(
      api.post<ApiResponse<{ url: string }>>("/auth/banner", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      }),
    );
  },
  updateGamePreferences: (gamePreferences: any[]) =>
    unwrap(api.put<ApiResponse<AuthPayload>>("/auth/game-preferences", { gamePreferences })),
};
...
export const postsApi = {
  ...
  create: (
    payload: Partial<FeedPost> & { content?: string; gameTag?: GameId; postType?: string },
  ) => unwrap(api.post<ApiResponse<FeedPost>>("/posts", payload)),
  get: (id: string) => unwrap(api.get<ApiResponse<FeedPost>>(`/posts/${id}`)),
  update: (id: string, payload: { content?: string; tags?: string[]; gameTag?: GameId }) =>
    unwrap(api.put<ApiResponse<FeedPost>>(`/posts/${id}`, payload)),
  delete: (id: string) => unwrap(api.delete<ApiResponse<null>>(`/posts/${id}`)),
  getByUser: (userId: string, page = 1) =>
    unwrap(api.get<ApiResponse<FeedResponse>>(`/posts/user/${userId}`, { params: { page } })),
  getSaved: (page = 1) =>
    unwrap(api.get<ApiResponse<FeedResponse>>("/posts/saved", { params: { page } })),
  ...
  comment: (id: string, text: string) =>
    unwrap(api.post<ApiResponse<any[]>>(`/posts/${id}/comment`, { text })),
  deleteComment: (postId: string, commentId: string) =>
    unwrap(api.delete<ApiResponse<any[]>>(`/posts/${postId}/comment/${commentId}`)),
  likeComment: (postId: string, commentId: string) =>
    unwrap(
      api.post<ApiResponse<{ liked: boolean; likesCount: number }>>(
        `/posts/${postId}/comment/${commentId}/like`,
      ),
    ),
  votePoll: (id: string, optionIndex: number) =>
    unwrap(api.post<ApiResponse<any>>(`/posts/${id}/poll/vote`, { optionIndex })),
};

// ─── SCRIMS ───────────────────────────────────────────
export const scrimsApi = {
  list: (params?: { game?: GameId; status?: string }) =>
    unwrap(api.get<ApiResponse<ScrimEvent[]>>("/scrims", { params })),
  ...
  create: (payload: Record<string, unknown>) =>
    unwrap(api.post<ApiResponse<ScrimEvent>>("/scrims", payload)),
  update: (id: string, payload: Record<string, unknown>) =>
    unwrap(api.put<ApiResponse<ScrimEvent>>(`/scrims/${id}`, payload)),
  delete: (id: string) => unwrap(api.delete<ApiResponse<null>>(`/scrims/${id}`)),
  register: (id: string, payload?: { teamName?: string; players?: string[] }) =>
    unwrap(api.post<ApiResponse<ScrimEvent>>(`/scrims/${id}/register`, payload || {})),
  updateRegistration: (scrimId: string, regId: string, status: string) =>
    unwrap(
      api.put<ApiResponse<ScrimEvent>>(`/scrims/${scrimId}/registrations/${regId}`, { status }),
    ),
  submitResults: (id: string, results: any[]) =>
    unwrap(api.post<ApiResponse<ScrimEvent>>(`/scrims/${id}/results`, { results })),
...
export const communityApi = {
  listings: (params?: { game?: GameId; type?: string; page?: number }) =>
    unwrap(
      api.get<ApiResponse<{ items: CommunityItem[]; total: number }>>("/community", { params }),
    ),
  get: (id: string) => unwrap(api.get<ApiResponse<CommunityItem>>(`/community/${id}`)),
  create: (payload: Record<string, unknown>) =>
    unwrap(api.post<ApiResponse<CommunityItem>>("/community", payload)),
  update: (id: string, payload: Record<string, unknown>) =>
    unwrap(api.put<ApiResponse<CommunityItem>>(`/community/${id}`, payload)),
...
export const clubsApi = {
  list: (params?: { game?: GameId; q?: string }) =>
    unwrap(api.get<ApiResponse<any[]>>("/clubs", { params })),
  get: (id: string) => unwrap(api.get<ApiResponse<any>>(`/clubs/${id}`)),
  create: (payload: Record<string, unknown>) =>
    unwrap(api.post<ApiResponse<any>>("/clubs", payload)),
  update: (id: string, payload: Record<string, unknown>) =>
    unwrap(api.put<ApiResponse<any>>(`/clubs/${id}`, payload)),
  ...
  createEvent: (clubId: string, payload: Record<string, unknown>) =>
    unwrap(api.post<ApiResponse<any>>(`/clubs/${clubId}/events`, payload)),
  updateEvent: (clubId: string, eventId: string, payload: Record<string, unknown>) =>
    unwrap(api.put<ApiResponse<any>>(`/clubs/${clubId}/events/${eventId}`, payload)),
  deleteEvent: (clubId: string, eventId: string) =>
    unwrap(api.delete<ApiResponse<any>>(`/clubs/${clubId}/events/${eventId}`)),
  toggleAttend: (clubId: string, eventId: string) =>
    unwrap(api.post<ApiResponse<any>>(`/clubs/${clubId}/events/${eventId}/attend`)),
  createPost: (clubId: string, payload: Record<string, unknown>) =>
    unwrap(api.post<ApiResponse<any>>(`/clubs/${clubId}/posts`, payload)),
...
export const clipsApi = {
  ...
  upload: (formData: FormData) =>
    unwrap(
      api.post<ApiResponse<any>>("/clips", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      }),
    ),
  update: (
    clipId: string,
    payload: { title?: string; description?: string; tags?: string[]; game?: GameId },
  ) => unwrap(api.put<ApiResponse<any>>(`/clips/${clipId}`, payload)),
  delete: (clipId: string) => unwrap(api.delete<ApiResponse<null>>(`/clips/${clipId}`)),
  like: (clipId: string) =>
    unwrap(api.post<ApiResponse<{ liked: boolean }>>(`/clips/${clipId}/like`)),
  comment: (clipId: string, text: string) =>
    unwrap(api.post<ApiResponse<any[]>>(`/clips/${clipId}/comment`, { text })),
  deleteComment: (clipId: string, commentId: string) =>
    unwrap(api.delete<ApiResponse<any[]>>(`/clips/${clipId}/comment/${commentId}`)),
};
...
export const achievementsApi = {
  ...
  add: (formData: FormData) =>
    unwrap(
      api.post<ApiResponse<any>>("/achievements", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      }),
    ),
  delete: (achievementId: string) =>
    unwrap(api.delete<ApiResponse<null>>(`/achievements/${achievementId}`)),
};

// ─── SCOUTING ─────────────────────────────────────────
export const scoutingApi = {
  players: (params?: Record<string, unknown>) =>
    unwrap(api.get<ApiResponse<PlayerSearchResponse>>("/scouting/players", { params })),
  save: (playerId: string) =>
    unwrap(api.post<ApiResponse<{ saved: boolean }>>(`/scouting/save/${playerId}`)),
  saved: () => unwrap(api.get<ApiResponse<any[]>>("/scouting/saved")),
  note: (playerId: string, notes: string) =>
    unwrap(api.put<ApiResponse<any[]>>(`/scouting/note/${playerId}`, { notes })),
  invite: (playerId: string, message?: string) =>
    unwrap(api.post<ApiResponse<any>>(`/scouting/invite/${playerId}`, { message })),
};
...
export const profileApi = {
  ...
  search: (params?: Record<string, unknown>) =>
    unwrap(api.get<ApiResponse<PlayerSearchResponse>>("/profile/search", { params })),
  top: (params?: { game?: GameId; limit?: number }) =>
    unwrap(api.get<ApiResponse<any[]>>("/profile/top", { params })),
  trackView: (userId: string) => unwrap(api.post<ApiResponse<null>>(`/profile/${userId}/view`)),
  update: (payload: Record<string, unknown>) =>
    unwrap(api.put<ApiResponse<any>>("/profile", payload)),
  create: (payload: Record<string, unknown>) =>
    unwrap(api.post<ApiResponse<any>>("/profile", payload)),
};
...
export const organizationsApi = {
  ...
  create: (formData: FormData) =>
    unwrap(
      api.post<ApiResponse<any>>("/organizations", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      }),
    ),
  update: (formData: FormData) =>
    unwrap(
      api.put<ApiResponse<any>>("/organizations", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      }),
    ),
  dashboard: (params?: { page?: number }) =>
    unwrap(api.get<ApiResponse<any>>("/organizations/dashboard/players", { params })),
  scouting: (params?: Record<string, unknown>) =>
    unwrap(api.get<ApiResponse<PlayerSearchResponse>>("/organizations/scouting", { params })),
  savePlayer: (playerId: string, notes?: string) =>
    unwrap(api.post<ApiResponse<null>>(`/organizations/save-player/${playerId}`, { notes })),
