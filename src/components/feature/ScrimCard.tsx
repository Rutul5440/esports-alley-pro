...
export function ScrimCard({ event, onRegister }: { event: ScrimEvent; onRegister?: () => void }) {
  ...
  const organizer = typeof event.organizer === "string" ? event.organizer : event.organizer?.username || "ConqLink";
    ...
    <article className="rounded-lg border border-border bg-card p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{organizer} · {gameName(event.game)}</p>
        </div>
        {event.kind === "tournament" ? <Trophy className="text-primary" /> : <ShieldCheck className="text-primary" />}
      </div>

      <div className="mt-4 flex items-center gap-2 text-sm text-muted-foreground">
        <CalendarDays size={15} /> {formatDate(date)}
      </div>
      {event.prize && <p className="mt-4 rounded-md border border-border bg-background/60 p-3 text-sm">{event.prize}</p>}
      ...
      {onRegister && (
        <button type="button" onClick={onRegister} className="mt-4 w-full rounded-md bg-gradient-gold px-4 py-2 text-sm font-semibold text-primary-foreground shadow-gold">
          Register
        </button>
      )}
    </article>
