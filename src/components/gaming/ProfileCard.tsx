    ...
    >
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {player.verified && <BadgeCheck size={16} className="text-gold shrink-0 fill-gold/10" />}
          </div>
          <p className="text-xs text-muted-foreground truncate font-mono">@{player.username}</p>
          <div className="mt-2.5"><RankBadge rank={player.rank} tier={player.tier} size="sm" /></div>
        </div>
      </div>

      <p className="mt-4 text-sm text-muted-foreground/80 line-clamp-2 h-10 leading-relaxed font-sans">{player.bio || "No career brief uploaded yet. Scouting index fully active."}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-1.5">
        {player.roles.slice(0, 3).map((r) => (
          <span key={r} className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded bg-white/[0.03] text-gold border border-white/5 font-semibold font-mono">
            {r}
          </span>
        ))}
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between text-xs text-muted-foreground/60 font-mono font-medium">
        <span className="inline-flex items-center gap-1"><MapPin size={12} className="text-gold/60" />{player.location || "Global"}</span>
        <span className="inline-flex items-center gap-1"><Users size={12} className="text-gold/60" />{player.followers.toLocaleString()} scouts</span>
      </div>
    </Link>
  );
}
